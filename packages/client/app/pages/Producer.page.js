...
const ProducerPage = () => {
  ...
  const onPeriodicBookComponentContentChange = debounce(
    changedContent => {
  ...
  const handleImageUpload = async incomingFile => {
    if (!canModify) {
      return showUnauthorizedActionModal(false)
    }

    const file =
      incomingFile?.originFileObj ||
      incomingFile?.file ||
      incomingFile?.blob ||
      incomingFile
  ...
  const debouncedSaveComments = debounce(
    variables => {
      addComments({
        variables,
      })
  ...
  useWebSocket(
    `${webSocketServerUrl}/locks`,
    {
      ...
      reconnectInterval: attempt => {
        const base = (heartbeatInterval?.config || 5000) + 500
        const next = Math.round(base * Math.pow(1.5, attempt))
