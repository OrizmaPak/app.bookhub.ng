...
module.exports = (
  ...
  if (notesType === 'chapterEnd') {
    chapterEndnotes = cheerio.load(
      `<p></p><aside class="footnotes"><h2 class="notes-title">Notes</h2></aside>`,
    )
  }
  ...
  if (hasNotesInner) {
    if (notesType === 'footnotes') {
      ...
      $('footnote').each((i, elem) => {
        ...
        const elementId = $elem.attr('id')
        noteNumberChpEnd += 1
        const elementContent = `${$elem.html()} <a href="#ref-${elementId}"><sup>[^]</sup></a>`
        ...
        const callout = $(
          `<a class="note-callout" id="ref-${elementId}" href="#${bookComponent.id}-${elementId}">${noteNumberChpEnd}</a>`,
        )
