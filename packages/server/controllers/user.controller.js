...
const createUserSession = async (userId, token) => {
  ...
  try {
    const existing = await UserSession.query().where({ token }).first()

    if (existing) {
      if (existing.revokedAt) {
        await UserSession.patchAndFetchById(existing.id, { revokedAt: null })
      }
    ...
    logger.warn(`[SESSION] could not persist session: ${e.message}`)
  }
}

const getBooleanApplicationParameter = async (area, fallback = true) => {
  const appParameter = await ApplicationParameter.query()
    .where({ context: 'bookBuilder', area })
    .first()

  if (!appParameter) return fallback

  if (typeof appParameter.config === 'boolean') return appParameter.config

  if (typeof appParameter.config === 'string') {
    if (appParameter.config === 'true') return true
    if (appParameter.config === 'false') return false

    try {
      const parsed = JSON.parse(appParameter.config)
      if (typeof parsed === 'boolean') return parsed
    } catch (e) {
      logger.warn(
        `[USER CONTROLLER] - failed to parse application parameter ${area}: ${e.message}`,
      )
    }
  }
...
const ketidaLogin = async input => {
  try {
    const signInEnabled = await getBooleanApplicationParameter(
      'signInEnabled',
      true,
    )

    if (!signInEnabled) {
      throw new Error('Sign-in is currently disabled. Please contact Datasphir.')
    }
    ...
    if (active && verified) {
      ...
      await createUserSession(result?.user?.id, result?.token)
      return result
    }
