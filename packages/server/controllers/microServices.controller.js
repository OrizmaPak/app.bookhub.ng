...
const { getPublicPreviewUrl } = require('../utilities/publicUrl')
    ...
    return new Promise((resolve, reject) => {
      callMicroservice(PAGEDJS, {
        ...
        .then(async ({ data }) => {
          ...
          const normalized =
            data && data.link
              ? { ...data, link: getPublicPreviewUrl(data.link) }
              : data
...
const flaxHandler = async (data, options) => {
  ...
  const publicFlaxUrl = process.env.FLAX_PUBLIC_URL || getServiceURL(FLAX)
