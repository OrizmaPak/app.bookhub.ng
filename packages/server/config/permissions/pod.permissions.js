...
const canEditBookAndRelevantAssets = async (userId, bookId, ctx) => {
  try {
...
const canInteractWithBookAndRelevantAssets = async (userId, bookId, ctx) => {
  try {
    const isAuthenticatedUser = await isAuthenticated(userId, ctx)
...
const isAuthenticatedRule = rule()(async (parent, args, ctx, info) => {
  try {
    ...
    return isAuthenticated(userId, ctx)
  } catch (e) {
    throw new Error(e.message)
  }
})
...
const getBookRule = rule()(async (_, { id: bookId }, ctx) => {
  try {
    ...
    return canInteractWithBookAndRelevantAssets(userId, bookId, ctx)
  } catch (e) {
    throw new Error(e.message)
  }
})
...
const signUpEnabledRule = rule()(async () => {
  try {
    /* eslint-disable global-require */
    const ApplicationParameter = require('../../models/applicationParameter/applicationParameter.model')
    /* eslint-enable global-require */

    const appParam = await ApplicationParameter.query()
      .where({ context: 'bookBuilder', area: 'signUpEnabled' })
      .first()

    if (!appParam) return true

    if (typeof appParam.config === 'boolean') return appParam.config

    if (appParam.config === 'true') return true
    if (appParam.config === 'false') return false

    try {
      const parsed = JSON.parse(appParam.config)
...
const permissions = {
  Query: {
    ...
    backAdminValidate: allow,
    backAdminStats: allow,
  ...
  Mutation: {
    ...
    backAdminLogoutAll: allow,
    backAdminSendEmail: allow,
    backAdminRevokeBookTransfer: allow,
  },
}
