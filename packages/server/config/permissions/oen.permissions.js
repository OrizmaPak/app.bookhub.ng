...
const allButTheAuthor = async (userId, bookId, ctx) => {
  try {
...
const deleteFilesRule = rule()(async (parent, { ids }, ctx, info) => {
  try {
    ...
    return canInteractWithBookAndRelevantAssets(userId, bookIds[0], ctx)
  } catch (e) {
    throw new Error(e.message)
  }
})
...
const ingestWordFileRule = rule()(async (parent, { bookId }, ctx, info) => {
  try {
    ...
    return allButTheAuthor(userId, bookId, ctx)
  } catch (e) {
    throw new Error(e.message)
  }
})
...
const updateKetidaTeamMembersRule = rule()(
  async (parent, { teamId }, ctx, info) => {
    try {
      ...
      return allButTheAuthor(userId, objectId, ctx)
    } catch (e) {
      throw new Error(e.message)
    }
  },
...
const permissions = {
  ...
  Mutation: {
    ...
    backAdminSetUserActive: true,
    backAdminSetAccess: true,
