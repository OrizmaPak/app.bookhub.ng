      ...
      (error, stdout, stderr) => {
        if (error) {
          if (error.killed && timeoutMs) {
            return reject(`Command timed out after ${timeoutMs}ms: ${command}`)
          }

          return reject(error.message || stderr || `Command failed: ${command}`)
        }
...
const getTemplates = async () => {
  try {
    ...
    const cloneTimeoutMs = Number.parseInt(
      process.env.TEMPLATE_CLONE_TIMEOUT_MS || '10000',
      10,
    )
    ...
    const results = await Promise.allSettled(
      normalizedTemplates.map(async templateDetails => {
        ...
        const command = `GIT_TERMINAL_PROMPT=0 git clone ${url} ${
          branch ? `--branch ${branch}` : ''
        } ./templates/${label}`

        await execute(command, { timeoutMs: cloneTimeoutMs })
        return label
      }),
    )
    ...
    if (failed.length) {
      logger.warn(
        `[templates] ${failed.length} template clone(s) failed or timed out. Proceeding with available templates.`,
      )
      failed.forEach(reason => logger.warn(`[templates] ${reason}`))
    }

    return successful
  } catch (e) {
    logger.error(`[templates] fatal error while fetching templates: ${e.message}`)
    return []
  }
}
