...
const { createGraphqlTestServer } = require('@coko/server')
...
describe('Book GraphQL Query', () => {
  ...
  beforeEach(async () => {
    ...
    testServer = await createGraphqlTestServer()
  })

  it('creates a book with just a title', async () => {
    const res = await createBookGraphQL(testServer, user.id, {
      resStructure: `{
    id
    authors{id}
    archived
  ...
  it('creates and then updates its podMetadata', async () => {
    let res = await createBookGraphQL(testServer, user.id, {})
    const bookData = res.data.createBook
    expect(res.errors).toBe(undefined)
  ...
  it('deletes the corresponding book settings when a book is deleted', async () => {
    const creationRes = await createBookGraphQL(testServer, user.id, {
      resStructure: '{id}',
    })

    const bookData = creationRes.data.createBook

    await deleteBookGraphQL(testServer, user.id, {
      resStructure: '{id}',
      id: bookData.id,
    })
