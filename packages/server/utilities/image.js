...
const replaceImageSource = async (content, filesFetcher) => {
  ...
  if (fileIds.length > 0) {
    ...
    $('img').each((i, elem) => {
      ...
      if (correspondingFile) {
        ...
        // Keep publishing resilient when a referenced file record is missing.
        $elem.attr('src', 'data:image/gif;base64,R0lGODlhAQABAAAAACw=')
        $elem.attr('alt', '')
      }
    })
  }
  ...
  // Ensure no image keeps an empty src, which breaks Flax optimization.
  $('img').each((i, elem) => {
    const $elem = $(elem)
    const src = ($elem.attr('src') || '').trim()
